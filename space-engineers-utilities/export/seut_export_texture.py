...
def export_material_textures(self, context, material):
    ...
    if 'CM' in nodes and os.path.exists(nodes['CM'].image.filepath):
        textures['cm'] = get_abs_path(nodes['CM'].image.filepath)

    if 'ADD' in nodes and os.path.exists(nodes['ADD'].image.filepath):
        textures['add'] = get_abs_path(nodes['ADD'].image.filepath)

    if 'NG' in nodes and os.path.exists(nodes['NG'].image.filepath):
        textures['ng'] = get_abs_path(nodes['NG'].image.filepath)
        
    if 'ALPHAMASK' in nodes and os.path.exists(nodes['ALPHAMASK'].image.filepath):
