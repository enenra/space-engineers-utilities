...
def export(self, context):
    ...
    if scene.seut.export_largeGrid or scene.seut.sceneType == 'character_animation' or scene.seut.sceneType == 'character':
        ...
        if grid_scale == 'small':
            ...
            if scene.seut.export_grid3To5Ratio:
...
def export_sbc(self, context):
    ...
    ratio3To5Scalar = 1.0 # default to doing nothing unless the 3to5 mode is detected
    ...
    if scene.seut.gridScale == 'large':
        ...
        if (abs(scene.seut.export_rescaleFactor - 3) < 0.01): # floating point comparison
            ratio3To5Scalar = 0.6 # Large grid block is going to be 3/5 of the expected size
        ...
        if (abs(scene.seut.export_rescaleFactor - 0.6) < 0.01): # floating point comparison
            ratio3To5Scalar = 3.0 # Small grid block is going to be 3 times larger than expected
    ...
    def_Size.set('x', str(scene.seut.bBox_X * ratio3To5Scalar))
    def_Size.set('y', str(scene.seut.bBox_Z * ratio3To5Scalar))   # This looks wrong but it's correct: Blender has different forward than SE.
    def_Size.set('z', str(scene.seut.bBox_Y * ratio3To5Scalar))
    ...
    if center_empty is not None:
        ...
        def_Center.set('x', str(round(ratio3To5Scalar * center_loc_x / grid_size)))
        def_Center.set('y', str(round(ratio3To5Scalar * center_loc_z / grid_size)))   # This looks wrong but it's correct: Blender has different forward than SE.
        def_Center.set('z', str(round(ratio3To5Scalar * center_loc_y / grid_size)))
    ...
    if len(scene.seut.mountpointAreas) > 0:
        ...
        for area in scene.seut.mountpointAreas:
            if area is not None:
                ...
                def_Mountpoint.set('b_StartX', str("{:.2f}".format(round(start_x * ratio3To5Scalar, 2))))
                def_Mountpoint.set('c_StartY', str("{:.2f}".format(round(start_y * ratio3To5Scalar, 2))))
                def_Mountpoint.set('d_EndX', str("{:.2f}".format(round(end_x * ratio3To5Scalar, 2))))
                def_Mountpoint.set('e_EndY', str("{:.2f}".format(round(end_y * ratio3To5Scalar, 2))))
