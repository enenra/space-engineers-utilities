...
# Changelog
* Added [#379](https://github.com/enenra/space-engineers-utilities/issues/379): Mark unpainted and ungrouped vertices in groups. (Beta 3)
* Added: `SEUT QuickTools` - shortcuts for various commonly used actions when making SE models. (Beta 1)
* Added: Material variation dropdown menu, allowing the user to quickly switch between versions of a material. (Beta 3)
* Added: Checkboxes to `Addon Preferences` to enable `Quick Tools` and `Animation Support`. (Beta 1)
* Changed: Switched all documentation links to instead link to the new [Space Engineers Official Wiki](https://spaceengineers.wiki.gg/). SEMREF will be retired, but all its content has been migrated to the SE wiki. (Beta 3)
...
* Improved: Channel settings on imported images. (Beta 3)
* Improved: Paint Color handling. This will unfortunately reset all paint color in your exisitng `BLEND`-files. (Beta 3)
* Improved: Sum up imported materials instead of reporting them one by one. (Beta 3)
...
* Fixed: Paths in exported logs sometimes not getting properly anonymized. (Beta 3)
* Fixed: `SEUT Material` being remapped to a linked one. (Beta 3)
* Fixed: Remap Materials sometimes erroring. (Beta 3)
* Fixed: Missing character weighting error not triggering (Beta 3)
